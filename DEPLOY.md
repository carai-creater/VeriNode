...
| 区分 | 内容 |
|------|------|
| **済（このリポジトリ）** | `POST /verify`（Serper / Google CSE）、HTTP **402** と `X-Payment-Link` / `X-Payment-Proof`、Stripe Checkout 発行・`cs_...` 検証、固定トークン方式、`GET /.well-known/ai-agent.json`、`POST /billing/checkout-session`、`POST /webhooks/stripe`（署名検証）、`Dockerfile` / `render.yaml`、`scripts/run_with_payment_gate.sh`、`pytest`（課金・検索未設定の分岐含む）、空環境変数の正規化、Stripe キーありで URL 不足時の **503** |
| **代行不可（当人必須）** | **Serper / Stripe / ホスト各社**へのログイン・アカウント作成。**API キー・Webhook シークレットの発行**と、**.env またはホストの Environment Variables への貼り付け**（秘密をチャットや Git に載せない）。**ブラウザで Checkout を開きテストカードで支払う**操作。**デプロイ先でのリポジトリ接続・ビルド・環境変数入力**。**独自ドメイン・DNS・証明書**（ホスト任せでも DNS は当人）。**Stripe ダッシュボードでの Webhook エンドポイント登録**、**Stripe CLI のインストールと `stripe listen`**（使う場合）。**Stripe Agentic Commerce 等への登録・審査・URL 提出**。**利用規約・特定商取引・プライバシー**など必要な法的表記の判断・掲載 |

ローカル用コマンド短縮: `make install` → `make test` → `make run`（課金試験は `make run-payment`）。

**作業を順番に進める**: [SETUP_STEPS.md](./SETUP_STEPS.md)。**Checkout・デプロイ・Webhook・Agentic Commerce を画面操作レベルで**は [GUIDE_CHECKOUT_DEPLOY_STRIPE.md](./GUIDE_CHECKOUT_DEPLOY_STRIPE.md)。
...
- `GET /.well-known/ai-agent.json` — 能力・認証手順・エンドポイント（A2A 系の一般的な Agent Card 形）。
...
- `GET /health` → `{"ok": true}`
- `GET /.well-known/ai-agent.json` → Agent Card JSON
